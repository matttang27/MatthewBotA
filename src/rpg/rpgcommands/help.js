...
module.exports = {
    ...
    usage: `${config_json_1.proPrefix[1]}help, or ${config_json_1.proPrefix[1]}help [command]`,
    example: `${config_json_1.proPrefix[1]}help ping`,
    perms: 4,
    execute(message, args, other) {
        let data = [];
        console.log;
        let rpgcommands = message.client;
        //sends command list if there are no arguments
        if (!args.length) {
            data.push("Here's a list of all my commands:");
            rpgcommands.forEach(command => {
                if (command.ownerOnly && message.author.id != config_json_1.ownerID) {
                    return;
                }
                else {
            ...
            data.push(`\nYou can send \`${config_json_1.proPrefix[0]}help [command]\` to get info on a specific command!`);
            data.push(`\nFor RPG commands, use ${config_json_1.proPrefix[1]}help instead!`);
        ...
        if (command.example)
            data.push("**Example: **" + `${command.example}`);
