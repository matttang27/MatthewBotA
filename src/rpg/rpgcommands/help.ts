import { ownerID, proPrefix } from "../../../config.json";
...
module.exports = {
	...
	usage: `${proPrefix[1]}help, or ${proPrefix[1]}help [command]`,
	example: `${proPrefix[1]}help ping`,
	perms: 4,
	...
	execute(message, args, other) {
		...
		if (!args.length) {
			...
			data.push(`\nYou can send \`${proPrefix[0]}help [command]\` to get info on a specific command!`);
			data.push(`\nFor RPG commands, use ${proPrefix[1]}help instead!`)
