...
function minutesToMessage(minutes) {
    ...
    let min = Math.floor(minutes - (days * 1440) - (hours * 60));
    var timer = "";
    if (days == 1) {
        timer += `1 day, `;
    }
...
function sendProfile(embed, user, userData) {
    ...
    embed.setColor('#0099ff')
        ...
        .addFields({ name: 'Balance', value: `$${userData.money}` }, { name: 'RPS W-L', value: `${userData.rpswon}-${userData.rpsplayed - userData.rpswon}`, inline: true }, { name: 'RPS Winrate', value: `${winrate}%`, inline: true }, { name: 'RPS Streak', value: userData.rpsstreak, inline: true }, { name: 'Married to:', value: `${married}` }, { name: 'Lovers', value: `${Object.keys(userData.lovers).length}`, inline: true }, { name: 'Rejections', value: `${Object.keys(userData.rejections).length}`, inline: true }, { name: 'Rejected', value: `${Object.keys(userData.rejected).length}`, inline: true }, { name: 'Age', value: `${(Math.floor((Date.now() - userData.timeCreated.toDate()) / 86400000))} days` })
        .setTimestamp()
