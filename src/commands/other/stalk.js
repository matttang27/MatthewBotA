module.exports = {
    ...
    async execute(message, args, other) {
        ...
        if (args[0] == "h" || args[0] == "history") {
            var pages = [];
            let h = e.history;
            let year, month, day, hour, minute, current1 = Object.keys(h);
            for (let i1 = current1.length - 1; i1 >= 0; i1--) {
                year = current1[i1];
                let current2 = Object.keys(h[year]);
                for (let i2 = current2.length - 1; i2 >= 0; i2--) {
                    month = current2[i2];
                    let current3 = Object.keys(h[year][month]);
                    for (let i3 = current3.length - 1; i3 >= 0; i3--) {
                        day = current3[i3];
                        let current4 = Object.keys(h[year][month][day]);
                        for (let i4 = current4.length - 1; i4 >= 0; i4--) {
                            hour = current4[i4];
                            let current5 = Object.keys(h[year][month][day][hour]);
                            for (let i5 = current5.length - 1; i5 >= 0; i5--) {
                                minute = current5[i5];
                                for (let i6 = h[year][month][day][hour][minute].length - 1; i6 >= 0; i6--) {
            ...
            for (let i = 0; i < new Date().getDay(); i++) {
                settings.labels.push(i.toString());
            }
            const chart = {
                type: 'line',
                data: {
