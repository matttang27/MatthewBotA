...
module.exports = {
    ...
    async execute(message, args, other) {
        ...
        async function makeEmbed(doujin, show) {
            ...
            if (temp)
                embed.setDescription(`**#${doujin.id}\n\n` + "AKA**: " + temp);
            console.log(parodies);
            embed.addFields({ name: "**parodies**", value: parodies.length ? parodies.join(" , ") : "none" }, { name: "**characters**", value: characters.length ? characters.join(" , ") : "none" }, { name: "**tags**", value: tags.length ? tags.join(" , ") : "none" }, { name: "**artists**", value: artists.length ? artists.join(" , ") : "none" }, { name: "**groups**", value: groups.length ? groups.join(" , ") : "none" }, { name: "**categories**", value: categories.length ? categories.join(" , ") : "none" }, { name: "**languages**", value: languages.length ? languages.join(" , ") : "none" }, { name: "**pages**", value: doujin.length }, { name: "**uploaded on**", value: uploaded }, { name: "**favourites**", value: doujin.favorites })
                .setURL(doujin.url)
        ...
        async function showSearch(sended, search, page, type) {
            ...
            var doujins = "\n";
            for (let i = 0; i < results.doujins.length; i++) {
