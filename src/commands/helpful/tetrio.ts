...
import { httpsGet } from '../../constants/functions.js'
)
module.exports = {
	args: [-1],
	name: "tetrio",
      ...
      for (let i in results) {
        if (!results[i]) {
          delete results[i]
			...
			let records = []
			Promise.allSettled(results).then(async (result: any) => {
			...
			let userdata = await Promise.all([httpsGet("https://ch.tetr.io/api/users/" + args[2].toLowerCase(),https)
			...
			var sended = await message.channel.send(embed)
			var userData:any = await updatePlayer(user.id,false)
