...
module.exports = {
    ...
    async execute(message, args, other) {
        ...
        if (args[0] == "leaderboard" || args[0] == "lb") {
            ...
            var lb = {};
            let results = players.map(p => updatePlayer(p, false));
            ...
            Promise.allSettled(results).then(async (result) => {
                ...
                if (args[1] == "tr" || !args[1]) {
                    ...
                    var text = "";
                    for (let i = 0; i < lineslb.length; i++) {
                    ...
                    var text = "";
                    for (let i = 0; i < blitzlb.length; i++) {
            ...
            let userdata = await Promise.all([(0, functions_js_1.httpsGet)("https://ch.tetr.io/api/users/" + args[2].toLowerCase(), https),
                (0, functions_js_1.httpsGet)("https://ch.tetr.io/api/users/" + args[2].toLowerCase() + "/records", https)]).then((values) => {
            ...
            var players = documentIds.filter(r => serverIds.indexOf(r) != -1);
            var lb = {};
            let results = players.map(p => updatePlayer(p, true));
            ...
            return new Promise(async (resolve) => {
                ...
                if (userData.user.success) {
                    ...
                    if (userData.user.cache.cached_until < Date.now() || force) {
                        userData.user = await (0, functions_js_1.httpsGet)("https://ch.tetr.io/api/users/" + username, https);
                    }
                    if (userData.records.cache.cached_until < Date.now() || force) {
                        userData.records = await (0, functions_js_1.httpsGet)("https://ch.tetr.io/api/users/" + username + "/records", https);
                    }
