	...
	try {
		...
		if (message.channel.type == "dm") {
			...
			if (!channel) {
				...
				var alias = []
				var guilds = []
				var guildlist = await bot.guilds.cache
				guildlist.forEach(async (g,i) => {
					try {
		...
		if (message.channel.type == "text") {
			...
			//Innovire Community stuff
			if (message.guild.id == "787368166430998529") {
				if (message.channel.name == "admin") {
					let ic = message.channel
					let perms = ic.permissionOverwrites
					console.log(perms)
...
async function syncEmotes() {
	...
	let emojis = await bot.emojis.cache
	emojis.forEach((e,i) => {
		e[e.name] = e.id
	})
