...
bot.on("messageCreate", async (message) => {
    try {
                ...
                if (message.channel.id == "837743995828174878") {
                    if (message.attachments.first()) {
                        if (!message.content) {
                            ...
                            const collector = message.channel.createMessageCollector({ filter: (m) => m.author.id == message.author.id, time: 30000, max: 1 });
                    ...
                    if (!(temp.split(" ").includes("matthew") ||
                        ...
                        for (i = 0; i < temp.split(" ").length; i++) {
                            if (praise.includes(temp.split(" ")[i])) {
                                message.channel.send("https:cdn.discordapp.com/attachments/720351714791915523/764105109536768020/unknown.png");
                                return;
                            }
                        }
                    }
        ...
        if (command.nsfw && (message.channel.type == "GUILD_TEXT" && message.channel.nsfw)) {
            message.reply("THIS IS A WHOLESOME AND SAFE CHANNEL! THAT COMMAND CANNOT BE USED HERE.");
            return;
        }
        ...
        if (command.status == "wip") {
            message.channel.send("That command is currently a WIP, please check again later (or pester Matthew to code it faster)");
            return;
        }
        else if (command.status == "closed") {
            message.channel.send("That command is currently closed. Either Matthew hasn't started coding, or it isn't needed.");
            return;
        }
        try {
